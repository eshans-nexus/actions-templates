## Distribute AMI (`actions/distribute-ami`)

### Overview
This action handles the cross-region distribution of a newly minted AMI. It takes a single source AMI and duplicates it across a specified list of target release regions, ensuring that both the AMI and its underlying EBS snapshots are made public. Finally, it generates a `RegionMap` JSON object intended for direct injection into a CloudFormation template.
...
2. **Distribution:** Initiates asynchronous `copy_image` requests via `boto3` to all target regions.
...
### Operational Modes
* **Real Mode:** Executes standard boto3 calls against AWS infrastructure.
...
- name: Distribute Built AMI
  id: distribute_ami
  uses: eshans-nexus/actions-templates/.github/actions/distribute-ami@main
  with:
